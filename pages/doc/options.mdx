...
Annotations describe how `textContent` in a `doc` should be marked.  This is achieved by specifying  the `start` and `end` offset of the annotation relative to the `textContent`.  Custom `classNames` and `styles` can be provided to customize annotations.  Overlapping annotations are also supported.

The following example demonstrates core features of annotations.  The `textContent` representation of the `doc` is provided to help visualize how annotations are processed against the `textContent`.
