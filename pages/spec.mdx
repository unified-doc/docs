...
Supports easy ways to convert between file formats by providing a supported `extension` argument (e.g. `.html`, `.txt`).  File data for the provided extension is returned.  The formal spec for `FileData` is provided below.
...
// {
  ...
  // type: 'text/html',
// }
console.log(doc.file('.txt')); // outputs txt file
// {
  // content: '> **some** markdown content',
  // extension: '.txt',
  // name: 'doc.txt',
...
```js
const content = '> **some** markdown content';
const searchOptions = { snippetOffsetPadding: 10 }
