...
<!-- https://github.com/unified-doc/unified-doc/blob/master/spec.md -->
...
Searches on the `text` content of a `doc` when a `query` string and configurable `options` is provided.  Returns `SearchResultSnippet`.  This method supports a simple and robust way to search on a `doc` irregardless of its underlying `mimeType`. Custom `searchAlgorithm` with the same unified interface using a shared `searchOptions` can be easily integrated.
