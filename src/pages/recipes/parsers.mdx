...
### Supported parsers
`unified-doc` infers the [mime type][] from the `filename` to determine which parser should be used.  The following mime types and parsers are supported, or will be supported for the following markup docment formats:
- [x] `text/html`: HTML parser
- [x] `text/markdown`: Markdown parser
- [ ] `text/csv`: CSV parser
- [ ] `text/xml`: XML parser
- [ ] `application/epub+zip`: EPUB parser
- [ ] `application/vnd.openxmlformats-officedocument.wordprocessing`: DOCX parser
- [ ] `application/mathml+xml`: MathML parser
